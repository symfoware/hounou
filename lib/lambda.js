...
export class Lambda {
    ...
    async makeTask() {
        await Promise.all([
            ...
            this.collectCurrentLayerInfo(this.config.Service)
        ]);
    ...
    async collectFunctionsInfo() {
        const wait = [];
        for (const func of this.config.getFunctions()) {
            wait.push(this.collectFunctionInfo(func.FunctionName));
        }
    ...
    async collectCurrentLayerInfo(layerName) {
        ...
        try {
            ...
            if (this.layerInfo.Description !== hash) {
                console.info(`${layerName} layer changed. create new layer.`);
                return;
            }
            ...
            console.info(`${layerName} layer not update. use exists layer.`);

        } catch(e) {
            console.info(`${layerName} layer not found. create new layer.`);
        }
    ...
    async deploy() {
        ...
        if (this.task.layer.operation === 'create') {
            ...
            const layerInfo = await this.createLayer(this.config.Service);
        ...
        for (const func of this.config.getFunctions()) {
            let mode = '';
            // 関数の新規作成
            if (this.task.functions[func.FunctionName] === 'create') {
                await this.createFunction(func.FunctionName, codeZip);
                mode = 'Create';

            // 内容の更新
            } else {
                // コード更新
                await this.updateFunction(func.FunctionName, codeZip);
                mode = 'Update';

            }

            console.info(`${mode} ${func.FunctionName} Function`);

            // バージョンの発行
            const version = await this.publishVersion(func.FunctionName, func.Description ?? '');
            console.info(`Publish ${func.FunctionName} Function Version ${version}`);
        }
    ...
    async createFunction(name, codeZip) {
        ...
        await this.waitFunctionUpdate(name, 'Create');
    }
    ...
    async updateFunction(name, codeZip) {
        ...
        // 関数更新待ち
        await this.waitFunctionUpdate(name, 'Update Code');
        ...
        await this.waitFunctionUpdate(name, 'Update Config');

    }

    // バージョン発行
    async publishVersion(name, description) {
        const input = {
            FunctionName: name, // required
    ...
    async createZipCode() {
        let ignore = [];
        if (this.config.Package && this.config.Package.Ignore) {
            ignore = this.config.Package.Ignore;
        }
    ...
    async createLayer(name) {
        ...
        for (const copyFile of copyFiles) {
            fs.copyFileSync(copyFile, path.join(nodejsdir, copyFile));
        }
        
        // node_modulesを取得
        // ここはPromiseにしないといけない
        const child = spawn('npm', 
            ['install', '--omit=dev', `--prefix=${nodejsdir}`, '--cpu=x86_64', '--os=linux'],
    ...
    // 関数が更新されるのを待機
    async waitFunctionUpdate(name, reason='Create') {
        const input = {
            FunctionName: name
        };
        const command = new GetFunctionConfigurationCommand(input);
        ...
        for (let i = 0; i < 100; i++) {
            ...
            if (response.LastUpdateStatus === 'Successful') {
                ...
                throw Error(`Function ${name} ${reason} failed.`);
            } else {
                console.info(`Waiting ${name} Function ${reason}...`);
            }

            await setTimeout(1000);
        }

        throw Error(`Wait ${name} Function ${reason} time out.`);
    }
    ...
    async clean() {
        ...
        // 設定ファイル必須項目チェック
        const prune = this.config.Custom?.Prune;
        if (!prune) {
            return;
        }
        ...
        for (const func of this.config.getFunctions()) {
            ...
            // 使用/未使用な関数バージョンを取得
            const [usedVersions, unusedVersions] = await this.getFunctionVersionUnused(func.FunctionName);

            // 未使用関数の削除実行
            while(unusedVersions.length > prune.Number) {
                const unused = unusedVersions.shift();
                await this.deleteFunction(func.FunctionName, unused.version);
            }
        ...
        // レイヤー情報取得
        const layerName = this.config.Service;
    ...
    async deleteFunction(name, version) {
        ...
        console.info(`Remove ${name} Function Version ${version}`);
    }
    ...
    async deleteLayer(layerName, version) {
        ...
        console.info(`Remove ${layerName} layer Version ${version}`);
